...
def prom_anual_kwhr(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			context = {"mensaje":"Hemos recibido la informacion, tu consumo promedio anual es de: ",
						"nombre": promedio,
						}
			lista = [1,2,3,4,5,6,7,8,9]
			return render(request,"cotizacion_residencial.html", {'lista':rut.calculaResidencial(promedio)})
	else:
